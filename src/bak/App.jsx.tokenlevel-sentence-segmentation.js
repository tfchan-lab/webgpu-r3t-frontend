...
function App() {
	...
	*/
	...
	const mainThreadHandler = (e) => {
		switch (e.data.status) {
			...
			case 'whisper_start':
				...
				recorderRef.current?.requestData();
				break;
			// Worker sends back raw output text immediately
			case 'whisper_raw_complete':
				console.log('[App] Received raw text from Whisper Worker');
				setText(e.data.output);
				setIsProcessing(false);
				break;

			// Worker sends back deduplicated text asynchronously
			case 'whisper_dedupe_complete':
				if (!e.data.output) return;
				console.log('[App] Received sentence from Whisper Worker');
				setWPrompt(e.data.updatedWPrompt);
				setInput(e.data.output);
				inputRef.current = e.data.output;
				if (TranslateWorker.current) {
					TranslateWorker.current.postMessage({
						type: 'translate',
						data: { text: e.data.output, src_lang: language, tgt_lang: targetLanguage }
					});
				}
	...
	const translate = async () => {
		...
		console.log('[App] Received translation');
