...
function App() {
	...
	const [ready, setReady] = useState(null); // TODO: Check if this is still in-use or just deprecated (other than in experiemental)
    const [loadingMessage, setLoadingMessage] = useState('');
    const [progressItems, setProgressItems] = useState([]);

    // Inputs and outputs
    const [text, setText] = useState(''); // Whisper output (gray)
	const [tps, setTps] = useState(null); // Token per second counter for Whisper
    const [input, setInput] = useState(''); // Translate input (from Whisper outputs)
	...
	const [confirmedText, setConfirmedText] = useState(''); // Confirmed transcription output (white)
	const [previousOutput, setPreviousOutput] = useState('');
	const [previousLCP, setPreviousLCP] = useState('');
	...
	const [useLocalTranslation, setUseLocalTranslation] = useState(false); // Default to server translation
	...
	const mainThreadHandler = (e) => {
		switch (e.data.status) {
			...
			case 'whisper_complete':
				...
				const currentOutput = e.data.output[0];
				//const currentOutput = trimLCP(e.data.output[0]);
				setText(currentOutput);
				setPreviousOutput(currentOutput);
				
				const lcp = findLCP(currentOutput, previousOutput);
				if (lcp.length > 0) {
					setPreviousLCP(lcp);
					const whiteText = confirmedText + ' ' + lcp;
					const grayText = currentOutput.replace(lcp, '');
					console.log(`White Text: ${whiteText}\nGray Text:  ${grayText}`);
					setConfirmedText(whiteText);
					setPreviousOutput(grayText);
					setText(grayText);
				}
				/*
				if (e.data.output[0] && TranslateWorker.current) {
					TranslateWorker.current.postMessage({
						type: 'translate',
						data: { text: text, src_lang: language, tgt_lang: targetLanguage }
					});
	...
	// Find the longest common prefix of two word sequences
	const findLCP = (words1, words2) => {
		console.log(`Previous: ${words2}\nCurrent: ${words1}`);
		let lcp = [];
		...
		let lcpWords = lcp.join('').trimEnd();
		console.log('Found LCP: ', lcpWords)
		return lcpWords;
	};
	
	/*
	const trimLCP = (words) => {
		if (confirmedText === '' && previousLCP === '') return words;
		
		let lcp1 = [];
		let lcp2 = [];
		
		if (confirmedText !== '') {
			const confirmedWords = confirmedText.split(' ');
			const inputWords = words.split(' ');
			
			// Find the largest match between any substring of confirmedText and the prefix of words
			let maxMatchLength = 0;
			for (let i = 0; i < confirmedWords.length; i++) {
				for (let j = i + 1; j <= confirmedWords.length; j++) {
					const confirmedSubstring = confirmedWords.slice(i, j).join(' ');
					const inputPrefix = inputWords.slice(0, j - i).join(' ');
					if (confirmedSubstring === inputPrefix && j - i > maxMatchLength) {
						maxMatchLength = j - i;
					}
				}
			}

			// Trim the largest match from the input words
			lcp1 = inputWords.slice(0,maxMatchLength);
		}
		
		for (let i = 0; i < Math.min(words.length, previousLCP.length); i++) {
			if (words[i] === previousLCP[i]) {
				lcp2.push(words[i]);
			} else {
				break;
			}
		...
		let lcpWords = (lcp1.length > lcp2.length) ? lcp1.join(' ') : lcp2.join('');
		let trimmedWords = words.replace(lcpWords, '');
		console.log('Trimmed Current: ', trimmedWords);
		return trimmedWords;
	};
	*/
